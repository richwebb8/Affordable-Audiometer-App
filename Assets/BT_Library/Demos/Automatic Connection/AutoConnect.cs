...
public class AutoConnect : MonoBehaviour
{
	...
	private void connect ()
	{
		 ...
		 */
		device.MacAddress = "00:06:66:7B:AB:2B";
    ...
    public void interuruptAudio()
    {
        interrupt.GetComponent<Text>().text = "Interrupt";

        if (device != null && !string.IsNullOrEmpty(interrupt.text))
        {
            device.send(System.Text.Encoding.ASCII.GetBytes(interrupt.text + (char)10));//10 is our seperator Byte (sepration between packets)
        }
    }
